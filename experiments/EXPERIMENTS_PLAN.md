...
자연어 인텐트 (사람이 말로 입력)
        ↓
[실험 1] LLM + RAG → FlowRule JSON 생성     ← 완료 (RAG 96.0%, Few-shot 대비 안정적)
        ↓
[실험 2] Static Validator → 오류/충돌 탐지   ← 완료 (스키마 100%, 충돌탐지 98.6%)
        ↓
[실험 3] Mininet Digital Twin → 가상 검증    ← 다음
        ↓
...
## 실험 2 — Static Validator (완료)

**폴더**: `experiments/2_static_validator/`
...
**구현 방법 2가지 비교 (완료)**
- 방법 A: 규칙 기반 (조건문으로 직접 비교) → **Accuracy 97.3%**
- 방법 B: LLM 기반 (NetIntent 방식, 두 FlowRule을 LLM에게 보여주고 판단) → **Accuracy 98.6%**

**결과**: FlowConflict-ONOS 74쌍 기준
| 방식 | Accuracy | Macro F1 |
|------|----------|----------|
| Rule-based | 97.3% | 0.970 |
| LLM-based | **98.6%** | **0.985** |
...
```
7/13         실험 1 (LLM+RAG) ✅ 완료
7/13         실험 2 (Static Validator) ✅ 완료
7/14 ~ 7/20  실험 4: XAI 설명 모듈 구현 + 평가
7/21 ~ 7/27  실험 3: Mininet DT (시간 여유 있을 때)
             + 전체 파이프라인 통합
7/28 ~ 8/3   실험 결과 정리, 표/그래프 작성
8/4  ~ 8/10  논문 작성 (Introduction, System Design, Evaluation)
8/11 ~ 8/17  논문 완성 (Related Work, Conclusion, 교정)
8/18 ~ 8/23  최종 교정 및 제출 준비
8/24         제출 마감
```
