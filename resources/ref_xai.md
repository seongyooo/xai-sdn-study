...
**제목:** Four Principles of Explainable Artificial Intelligence
**저자:** P. Jonathon Phillips, Carina A. Hahn, Peter C. Fontana, Amy N. Yates, Kristen Greene, David A. Broniatowski, Mark A. Przybocki
**출처 (학회/저널):** NIST Interagency Report 8312 (NISTIR 8312)
**연도:** 2021
**링크:** https://doi.org/10.6028/NIST.IR.8312

**요약:**
NIST(미국 국립표준기술연구원)가 제시하는 설명 가능한 AI 시스템의 4가지 근본 원칙. AI 커뮤니티, 심리학, 공학의 학제간 관점을 통합하여 도출. XAI 시스템 설계의 이론적 기반으로 널리 인용되는 표준 문서.
...
- **4가지 XAI 원칙**:
  ...
  | 원칙 | 정의 | 우리 시스템에서의 구현 |
  |------|------|----------------------|
  | **Explanation** | 출력과 프로세스에 대한 근거/증거 제공 | FlowRule 생성 근거 + RAG 참조 문서 제시 |
  | **Meaningful** | 사용 대상(intended consumer)이 이해할 수 있는 설명 | 네트워크 운영자 수준의 자연어 설명 (LLM) |
  | **Explanation Accuracy** | 설명이 시스템의 실제 처리 과정을 정확히 반영 | SHAP 기반 실제 기여도 측정 (추정 아님) |
  | **Knowledge Limits** | 설계된 조건 내에서만 동작, 신뢰도 낮으면 동작 거부 | Static Validator + DT 검증 실패 시 재생성 |

- **5가지 설명 카테고리**: 시스템 기반(System-based) / 결과 기반(Outcome-based) / 입력 기반 / 사례 기반 / 규칙 기반
- **사용자별 다른 설명 필요**: 개발자, 정책 입안자, 최종 사용자가 요구하는 설명 유형이 다름
- **과정 기반 vs 결과 기반 설명**: 모델이 어떻게 결정했는지(과정) vs. 무엇을 결정했는지(결과) 모두 중요
- **공개 리포트**: 무료 다운로드 가능 (https://doi.org/10.6028/NIST.IR.8312)

**내 연구와의 관련성:**
- **XAI 모듈 설계 이론 기반**: 우리 XAI 레이어가 4원칙을 충족하는지 검증하는 프레임워크로 활용
  - Explanation: SHAP 값 + LLM 설명 텍스트 → ✓
  - Meaningful: 네트워크 운영자 대상 자연어 설명 → ✓
  - Explanation Accuracy: SHAP (수학적 기여도, 사후 근사 아님) → ✓
  - Knowledge Limits: 검증 실패 시 FlowRule 배포 거부 → ✓
- **논문 XAI 섹션 도입부**: "본 연구의 XAI 모듈은 NIST의 4원칙을 기반으로 설계됐다"고 명시하면 이론적 정당성 확보
- **평가 지표 설계**: Meaningful 원칙 → 운영자 대상 Usefulness/Correctness 인간 평가 설계 근거
- **인용 가치**: NIST 공식 문서로 권위 있는 출처 — 학술지 심사자에게 신뢰도 높음
